...
import { fetchNodeTypeStyles } from '@/services/nodeTypeStyleService';
...
function nodeInfoHtml(
  node: MapNode,
  styles: Record<string, NodeTypeStyle>
): string {
  const title = escapeHtml(node.label);
  const kind = escapeHtml(node.type);
  const radius = getNodeRadius(node, styles);

  const desc = node.description
    ? `<div class="map-iw-desc">${escapeHtml(node.description)}</div>`
    : '';

  return `
    <div class="map-iw">
      <div class="map-iw-title">${title}</div>
      <div class="map-iw-type">${kind}</div>
      <div class="map-iw-radius">Influence: ${radius}m (max radius)</div>
      ${desc}
    </div>
  `;
}
...
export default function GoogleMapView() {
  ...
  const [nodeTypeStyles, setNodeTypeStyles] =
    useState<Record<string, NodeTypeStyle>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const [fetchedNodes, fetchedStyles] =
          await Promise.all([
            getMapNodes(),
            fetchNodeTypeStyles(),
          ]);
        ...
        const styleMap = toNodeTypeStyleMap(fetchedStyles);

        setNodes(fetchedNodes);
        setNodeTypeStyles(styleMap);
        setSelectedTypes(
          new Set(Object.keys(styleMap))
        );
      } catch {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    if (
      !map ||
      nodes.length === 0 ||
      Object.keys(nodeTypeStyles).length === 0
    ) {
      return;
    }
    ...
    const halos =
      createRadiantNodeHalosOverlay(filteredNodes, nodeTypeStyles);
      ...
      for (const node of filteredNodes) {
        const marker =
          new AdvancedMarkerElement({
            ...
            title: node.label,
            content: markerContentForType(
              getNodeColor(node.type, nodeTypeStyles)
            ),
          });
  ...
  const handlePlaceByCoordinates = useCallback(
